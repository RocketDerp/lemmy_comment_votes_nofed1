...
export async function setupLiveLogin() {
  ...
  if (process.env.LEM_USERNAME0) {
    ...
    if (loginResult.jwt && loginResult.jwt.length > 10) {
      ...
      console.error(
        "login failed, do not set username environment variable or fix password",
      );
      process.exit(10);
    }
  }
...
export async function setToBotAccount(account: API) {
  ...
  if (account.auth) {
    ...
    if (site.my_user) {
      ...
      let stripActor = site.site_view.site.actor_id
        .replace("https://", "")
        .replace("/", "");
      ...
      if (personObject) {
        ...
        if (!a) {
          ...
          console.log(
            "not a bot account, setting as bot since it is a live server",
          );
          let saveResult = await saveUserSettings(account, {
            auth: account.auth,
            bot_account: true,
            bio:
              "BulletinTree.com special bot account for local server stress testing.\n\n" +
