...
export let alpha_user_casual0: LemmyHttp;
...
*/
export async function registerUserClient(withapi: LemmyHttp, username: string) {
  let jwt;
  try {
    let formAlpha: Login = {
    ...
    let resAlpha = await alpha.login(formAlpha);
  ...
  let newUser: LemmyHttp = new LemmyHttp(alphaUrl);
  newUser.setHeaders({ auth: jwt ?? "" });
// ToDo: set cookies
  return newUser;
}
...
export async function createTargetCommunity(account: LemmyHttp) {
...
export async function loopActionSetA(
  ...
  for (let i = 0; i < 13; i++) {
    ...
    let postRes = await account.createPost(form);
...
export async function loopActionSetB(
  ...
  const name = targetCommunityName;
  // await resolveCommunity(account, name);
  let communityRes = await account.getCommunity({ name: name });
  expect(communityRes.community_view.community.name).toBe(name);
...
export async function manyCommunitiesManyPosts(account: LemmyHttp) {
...
export async function manyPostsWithAFewCommentsSameCommunity(account: LemmyHttp) {
  const postLoop = 25;
...
export async function nestedCommentsOnMostRecentPostsSpecificCommunityA(
  ...
  let posts = await account.getPosts(form);
...
export async function getPostsForTargetCommunity(
  ...
  let postsResult = await account.getPosts(form);
  if (!bypass_expect_a) {
