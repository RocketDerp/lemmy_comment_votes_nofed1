...
impl<'a> PostQuery<'a>
 {
  ...
  {
    let conn = lemmy_db_schema::utils::get_conn(pool).await?;
...
async fn meat_run_list(mut conn: DbConn<'_>, options: PostQuery<'_>) -> Result<Vec<<PostView as JoinView>::JoinTuple>, Error> {
    // let options = self;
    //let conn = pool;
