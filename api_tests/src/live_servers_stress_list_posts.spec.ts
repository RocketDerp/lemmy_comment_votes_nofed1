...
import { getPostsForTargetCommunity, setTargetCommunityName } from "./shared_benchmark";
import { live_user0, live_user0name, setupLiveLogin, target_server } from "./shared_live_servers";
...
test('using logged-in account, confirm live user info', async () => {
  let siteRes = await live_user0.client.getSite({auth: live_user0.auth});
  console.log("getSite user info %s", siteRes.my_user?.local_user_view.person.name);
...
test('setup not-logged-in, anonymous user. set targetCommunityName', async () => {
...
test('using logged-in account, query target community by new', async () => {
  await getPostsForTargetCommunity(live_user0, 5, "New");
}, 2 * 60 * 60 * 1000);


test('using anonymous account, query target community by new', async () => {
  await getPostsForTargetCommunity(anonymous_user0, 5, "New");
}, 2 * 60 * 60 * 1000);

test('using logged-in account, query target community by Hot', async () => {
  await getPostsForTargetCommunity(live_user0, 5, "Hot");
}, 2 * 60 * 60 * 1000);


test('using anonymous account, query target community by Hot', async () => {
