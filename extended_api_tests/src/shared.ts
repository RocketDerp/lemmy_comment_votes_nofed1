...
export async function setupLogins() {
  ...
  let editSiteForm: EditSite = {
    ...
    rate_limit_message: 9999,
    rate_limit_post: 9999,
    rate_limit_register: 999,
    rate_limit_image: 999,
    rate_limit_comment: 9999,
...
export async function followCommunity(
  ...
  await waitUntil(
    ...
    g => {
      if (g.community?.community.local) {
        // 'pending' bug on local community still in lemmy 2023-09-22?
        return true;
      } else {
        return g.community?.subscribed === (follow ? "Subscribed" : "NotSubscribed")
      }
    },
  );
