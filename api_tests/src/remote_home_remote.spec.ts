...
import { Community, GetCommentsResponse, GetCommunity, GetCommunityResponse, GetPosts, GetPostsResponse, HideCommunity, ModHideCommunity, ModHideCommunityView, PostResponse, PostView } from "lemmy-js-client";
import { registerUser, alpha, gamma, API, beta, createCommunity, followCommunity, resolveCommunity, setupLogins, getSite, resolvePerson, createPost, createComment, getComments, getCommentParentId, featurePost, lockPost, removePost, removeComment, banPersonFromCommunity, likeComment, likePost } from "./shared";
...
test("beta creates new community, becoming moderator", async () => {
    let betaCommunityRes = await createCommunity(beta_user_creator_mod, "enter_shikari");
    betaCommunityHome = betaCommunityRes.community_view.community;

    let communityFresh = await getCommunityResponse(beta_user_creator_mod, betaCommunityHome.id);
...
test("alpha and gamma instances discover newly created beta instance community", async () => {
    ...
    let alphaCommunityTemp = (await resolveCommunity(alpha_user_non_mod, searchShort)).community;  
    if (!alphaCommunityTemp) {
        throw "Missing community on alpha";
    }
    alphaCommunityRemote = alphaCommunityTemp.community;

    let gammaCommunityTemp = (await resolveCommunity(gamma_user_non_mod, searchShort)).community;  
...
test("not joining community, gamma non-admin user creates post and comment, before any subscribers", async () => {
    gamma_user_non_subscriber = await registerUserClient(gamma, "gamma_nonsubscriber0");
    // without any subscribers on alpha, it won't go to alpha
    let gammaPost0Res = await createPost(gamma_user_non_subscriber, gammaCommunityRemote.id);
    let gammaComment0Res = await createComment(gamma_user_non_subscriber, gammaPost0Res.post_view.post.id);
    ...
    let alphaBeforeSubscribersPosts = await getCommunityPostsFromListNew(alpha_user_non_mod, communityNameFull);
...
test("2 alpha and 2 gamma users subscribe to community", async () => {
    ...
    let freshCommunityHome = await getCommunityResponse(beta_user_creator_mod, betaCommunityHome.id);
...
test("not joining community, gamma non-mod user creates another post and comment, after other subscribers", async () => {
    ...
    let communityNameFull = betaCommunityHome.name + "@lemmy-beta";
    let alphaAfterSubscribersPosts = await getCommunityPostsFromListNew(alpha_user_non_mod, communityNameFull);
...
test("beta makes 1 gamma and 1 alpha user moderators of community", async () => {
    ...
    let alphaPersonOnBeta = (await resolvePerson(beta_user_creator_mod, apShortname)).person;
    ...
    let gammaPersonOnBeta = (await resolvePerson(beta_user_creator_mod, apShortnameGamma)).person;
    ...
    let communityFresh = await getCommunityResponse(alpha_user_mod, alphaCommunityRemote.id);
    expect(communityFresh.moderators.length).toBe(3);

    // check from gamma remote instance to see moderator additions replicated
    let gammaCommunityFresh = await getCommunityResponse(gamma_user_mod, gammaCommunityRemote.id);
    expect(gammaCommunityFresh.moderators.length).toBe(3);

    // check moderator usernames
    expect(gammaCommunityFresh.moderators[1].moderator.name).toBe("alpha_powermod0");
    expect(gammaCommunityFresh.moderators[2].moderator.name).toBe("gamma_powermod0");
});
...
export async function getCommunityPostsFromListNew(
  ...
  export async function getAllPostsFromListNew(
    api: API,
    local: boolean,
  ): Promise<GetPostsResponse> {
    let form: GetPosts = {
...
test("non-moderator user of alpha creates post + comment, non-moderator of gamma validates", async () => {
    ...
    let alphaPost0Res = await createPost(alpha_user_non_mod, alphaCommunityRemote.id);
    let alphaComment0Res = await createComment(alpha_user_non_mod, alphaPost0Res.post_view.post.id);
    ...
    expect(alphaComment0Res.comment_view.comment.ap_id).toBe(gammaPostComments.comments[0].comment.ap_id);
...
let betaLocatedPost = await findPostFromListNew(beta_user_creator_mod, communityNameFull, alphaPost0Res.post_view.post.ap_id);

    // moderator AND CREATOR of community on beta features the post in the community
    let modActionResult = await featurePost(beta_user_creator_mod, true, betaLocatedPost.post);
    ...
    expect(alphaPost0Res.post_view.post.ap_id).toBe(gammaPostComments.comments[0].post.ap_id);
...
test("SHORT OF BUG diagnostic try1: non-moderator user of alpha creates post, moderator of alpha community-features, beta reads it", async () => {
    ...
    let modActionResult = await featurePost(alpha_user_mod, true, alphaPost0Res.post_view.post);
    expect(modActionResult.post_view.post.featured_community).toBe(true);
    ...
    // check the comment ap_id equality out of band
    expect(alphaComment0Res.comment_view.comment.ap_id).toBe(betaPostComments.comments[0].comment.ap_id);
    // check the post ap_id equality out of band.
    expect(alphaPost0Res.post_view.post.ap_id).toBe(betaPostComments.comments[0].post.ap_id);
...
test("non-moderator user of alpha creates post, moderator of alpha locks post, gamma reads it", async () => {
    ...
    let modActionResult = await lockPost(alpha_user_mod, true, alphaPost0Res.post_view.post);
...
test("non-moderator user of alpha creates post, moderator of alpha removes post, gamma checks for post", async () => {
    ...
    let modActionResult = await removePost(alpha_user_mod, true, alphaPost0Res.post_view.post);
...
test("non-moderator user of alpha creates post+comment, moderator of alpha removes comment, gamma checks for comment", async () => {
    ...
    // check the comment ap_id equality out of band
    expect(alphaComment0Res.comment_view.comment.ap_id).toBe(gammaPostComments0.comments[0].comment.ap_id);
    // check the post ap_id equality out of band.
    expect(alphaPost0Res.post_view.post.ap_id).toBe(gammaPostComments0.comments[0].post.ap_id);

    // moderator of community on alpha removes the comment
    let modActionResult = await removeComment(alpha_user_mod, true, alphaComment0Res.comment_view.comment.id);
...
test("alpha moderator creates comment 'speaking as moderator', gamma validates", async () => {
    ...
    let alphaComment0Res = await createComment(alpha_user_mod, alphaPost0Res.post_view.post.id);
...
async function doBanUnbanUser(with_remove_data: boolean) {
    ...
    let beforeBanPosts = await getCommunityPostsFromListNew(alpha_user_observer, communityNameFull);
    ...
    let apShortname = `@${site.my_user.local_user_view.person.name}@lemmy-alpha:8541`;
    let alphaPersonOnGamma = (await resolvePerson(gamma_user_mod, apShortname)).person;
    ...
    let modActionResult = await banPersonFromCommunity(gamma_user_mod, alphaPersonOnGamma.person.id, gammaCommunityRemote.id, with_remove_data, true);

    // confirm the user who was banned is unable to post in community
    await expect(createPost(alpha_user_non_mod, alphaCommunityRemote.id)).rejects.toBe("banned_from_community");

    // this user in precvious test had several posts and comments in the community of focus
    // grab a list after ban
    let afterBanPosts = await getCommunityPostsFromListNew(alpha_user_observer, communityNameFull);
    ...
    let modActionResult1 = await banPersonFromCommunity(gamma_user_mod, alphaPersonOnGamma.person.id, gammaCommunityRemote.id, with_remove_data, false);

    let afterUnbanPosts = await getCommunityPostsFromListNew(alpha_user_observer, communityNameFull);
    if (with_remove_data) {
...
test("gamma user creates new post, alpha checks if replication turned off for community", async () => {
    ...
    let alphaBeforePosts = await getCommunityPostsFromListNew(alpha_user_observer, communityNameFull);
    expect(alphaBeforePosts.posts.length).toBe(3);

    let gammaPost0Res = await createPost(gamma_user_non_subscriber, gammaCommunityRemote.id);
    let gammaComment0Res = await createComment(gamma_user_non_subscriber, gammaPost0Res.post_view.post.id);

    let alphaAfterPosts = await getCommunityPostsFromListNew(alpha_user_observer, communityNameFull);
...
test("alpha has no subscribers to community, replication confirmed off by previous test, alpha user posts", async () => {
    ...
    let betaBeforePosts = await getCommunityPostsFromListNew(beta_user_creator_mod, communityNameFull);
    ...
    let betaAfterPosts = await getCommunityPostsFromListNew(beta_user_creator_mod, communityNameFull);
...
test("leave mod team of community from alpha, validate if gamma instance gets the moderator removal", async () => {
    // issue https://github.com/LemmyNet/lemmy/issues/3760

    // note list on gamma before self-removal of moderator.
    let communityBefore = await getCommunityResponse(gamma_user_non_mod, gammaCommunityRemote.id);
    ...
    let communityAfter = await getCommunityResponse(gamma_user_non_mod, gammaCommunityRemote.id);
    expect(communityAfter.moderators.length).toBe(communityBefore.moderators.length - 1);
    expect(communityAfter.moderators.length).toBe(2);
});

test.skip("behavior of censorship replication", async () => {
    // see comment, create code to document behavior and changes to behavior
    // https://lemmy.ml/comment/2118625
});

test("hiding a community", async () => {
    // https://join-lemmy.org/api/interfaces/HideCommunity.html
    let betaCommunityRes = await createCommunity(beta_user_creator_mod, "hidden_community0");
    let betaCommunity2Home = betaCommunityRes.community_view.community;

    let searchShort = `!hidden_community0@lemmy-beta:8551`;

    let alphaCommunityTemp = (await resolveCommunity(alpha_user_non_mod, searchShort)).community;  
    if (!alphaCommunityTemp) {
        throw "Missing community on alpha";
    }
    let alphaCommunity2Remote = alphaCommunityTemp.community;

    let gammaCommunityTemp = (await resolveCommunity(gamma_user_non_mod, searchShort)).community;  
    if (!gammaCommunityTemp) {
        throw "Missing community on gamma";
    }
    let gammaCommunity2Remote = gammaCommunityTemp.community;

    await followCommunity(alpha_user_mod, true, alphaCommunity2Remote.id)
    await followCommunity(alpha_user_non_mod, true, alphaCommunity2Remote.id);
    await followCommunity(gamma_user_mod, true, gammaCommunity2Remote.id)
    await followCommunity(gamma_user_non_mod, true, gammaCommunity2Remote.id);
    let freshCommunityHome = await getCommunityResponse(beta_user_creator_mod, betaCommunity2Home.id);
    expect(freshCommunityHome.community_view.counts.subscribers).toBe(5);

    let gammaPost0Res = await createPost(gamma_user_mod, gammaCommunity2Remote.id);
    let gammaComment0Res = await createComment(gamma_user_mod, gammaPost0Res.post_view.post.id);

    // from alpha, what does "All" look like
    let alphaBeforeHidden = await getAllPostsFromListNew(alpha_user_observer, false);
    expect(alphaBeforeHidden.posts.length).toBe(5);

    // beta community creator grants mod to gamma mod
    // the API object doesn't really know who they are, identity crisis
    let siteGamma = await getSite(gamma_user_mod);
    if (!siteGamma.my_user) {
        throw "Missing site user on gamma";
    ...
    await beta_user_creator_mod.client.addModToCommunity({
        community_id: betaCommunity2Home.id,
        person_id: gammaPersonOnBeta.person.id,
        added: true,
        auth: beta_user_creator_mod.auth
    });
    ...
    // gamma_user_mod.client.hideCommunity
    let dothis = false;
    if (dothis) {
        // remote moderator on gamma hides the comunity
        let a : HideCommunity;
        let b : ModHideCommunity;
        let c : ModHideCommunityView;
        // I find route, community/hide
        a = {
            community_id: gammaCommunity2Remote.id,
            hidden: true,
            reason: "testing",
            auth: gamma_user_mod.auth
        };
        let params = {
            serverChoice0: 'http://127.0.0.1:8561/',
            serverURLpath0: "api/v3/community/hide",
            bodyJSON0: JSON.stringify(a),
        }
        let modActionResult = await serverFetchJSON0(params);
        console.log(modActionResult);
    } else {
        let connectParam = "http://127.0.0.1:8561/" + "api/v3/community/hide?"
            + "community_id=" + gammaCommunity2Remote.id
            + "&hidden=true"
            + "&reason=" + "testing"
            + "&auth=" + gamma_user_mod.auth
        ...
        let connectParam1 = "http://127.0.0.1:8561/" + "api/v3/community/list?"
            + "community_id=" + gammaCommunity2Remote.id
            + "&hidden=true"
            + "&reason=" + "testing"
        ;
