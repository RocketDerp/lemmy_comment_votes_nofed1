    ...
    "post_like"."score",
     ...
     "post_aggregates"."comments")
      FROM (
       ...
       LEFT OUTER JOIN "community_person_ban" ON 
          (("post_aggregates"."community_id" = "community_person_ban"."community_id") AND ("community_person_ban"."person_id" = "post_aggregates"."creator_id"))
          )
       ...
       WHERE (
          ((((
          (
                 (("community"."removed" = false) AND ("post"."removed" = false))
...
-- HTTP request{http.method=GET http.scheme="http" http.host=lemmy-alpha:8541 http.target=/api/v3/user otel.kind="server" request_id=3cc56e58-a570-492d-9a67-11158e3ce4bd}:read_person{data=Query(GetPersonDetails { person_id: None, username: Some("HCE"), sort: Some(New), page: Some(1), limit: Some(20), community_id: None, saved_only: None, auth: Some(Sensitive) })}: lemmy_db_views::post_view: Post View Query: Query { sql:

/*
This is from listing a single user's profile, which takes several seconds to execute, even on an instant rerun/refresh and no data change at all (idle system).
why is featured in sort order on a profile listing?
*/

/*

SELECT 
   "post"."id", "post"."name", "post"."url", "post"."body", "post"."creator_id", "post"."community_id", "post"."removed", "post"."locked", "post"."published", "post"."updated", "post"."deleted", "post"."nsfw", "post"."embed_title", "post"."embed_description", "post"."thumbnail_url",
   ...
   "community"."id", "community"."name", "community"."title", "community"."description", "community"."removed", "community"."published", "community"."updated", "community"."deleted",
   ...
   "community"."followers_url", "community"."inbox_url", "community"."shared_inbox_url", "community"."hidden", "community"."posting_restricted_to_mods", "community"."instance_id",
...
WHERE (((((((
  ...
  AND ("community_block"."person_id" IS NULL))
  AND ("person_block"."person_id" IS NULL))
ORDER BY "post_aggregates"."featured_local" DESC , "post_aggregates"."published"
DESC  LIMIT 1
OFFSET 0
;


  -- binds: [3, 3, 3, 3, 3, 3, 3, 3, 2, false, false, false, false, 3, false, false, 20, 0]
...
SELECT * FROM person_block LIMIT 10;
