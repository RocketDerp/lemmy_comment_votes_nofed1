...
import { createNoLinkPost, getPostsForTargetCommunity, registerUserClient, setTargetCommunityName } from "./shared_benchmark";
...
export async function sim_create_accounts(admin_account: LemmyHttp) {
    ...
    if (u.join) {
      for (let c = 0; c < u.join.length; c++) {
        ...
        try {
          ...
          console.error("EXCEPTION failure to resolve or follow community %s", name);
          console.log(e0);
          process.exit(77);
        }
      }
...
export async function set_show_nsfw_for_accout(account: LemmyHttp) {
  try {
    let a = await account.getSite();
    if (a.my_user?.local_user_view.local_user.show_nsfw) {
      ...
      await account.saveUserSettings(
        {
          //auth: account.auth,
          show_nsfw: true,
