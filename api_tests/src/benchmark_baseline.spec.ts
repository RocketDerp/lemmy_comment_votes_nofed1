...
import { alpha_user_casual0, getPostsNewMax, loopActionSetA, nestedCommentsOnMostRecentPosts, setupBenchmarkLogins } from "./shared_benchmark";
...
test.skip("may as well study the content", async () => {
      ...
      for (let i = 0; i < posts.posts.length; i++) {
        ...
        console.log("%s %s %s", post.post.ap_id, post.post.local, post.post.community_id, post.post.name);
      }
});
...
*/
test("variability due to quantity of comments on post", async () => {
